...
Body = Frame(root, width=1300, height=900, bg="#d6d6d6")
Body.pack(pady=20, padx=20)

LHS = Frame(Body, width=440, height=520, bg="#f4f5f5",
...
l5 = Label(LHS, text=f"Total RAM installed:{round(psutil.virtual_memory().total/1000000000,2)} GB",
           bg="#f4f5f5", font=("Acumin Variable Concept", 8), justify="center")
l5.place(x=15, y=355)
...
RHS = Frame(Body, width=490, height=280, bg="#f4f5f5",
            highlightbackground="#adacb1", highlightthickness=1)
RHS.place(x=455, y=15)
...
######### ------this show green battery icon img----#######
    battery_label = Label(RHS, background='#f4f5f5')
    battery_label.place(x=10, y=55)
...
############### ---------THIS IS FOR BATTERY PERCENATAGE--------#############
lbl = Label(RHS, font=("Acumin Variable Concept", 30, 'bold'), bg='#f4f5f5')
lbl.place(x=240, y=10)

############### ---------THIS IS FOR plug in:false--------#############
lbl_plug = Label(RHS, font=("Acumin Variable Concept", 8), bg='#f4f5f5')
lbl_plug.place(x=10, y=120)

############### ---------THIS IS FOR TIME REMAINING--------#############
lbl_time = Label(RHS, font=("Acumin Variable Concept", 10), bg='#f4f5f5')
lbl_time.place(x=240, y=90)
none()


################# --------SPEAKER--------######################

lbl_speaker = Label(RHS, text="Speaker:", font=(
    'arial', 15, 'bold'), bg="#f4f5f5")
...
######## ---THIS IS FOR VOLUME POINTER SIGN------##########
volume = ttk.Scale(RHS, from_=60, to=0, orient='horizontal',
                   command=volume_changed, variable=volume_value)

volume.place(x=150, y=158)
volume.set(20)

####### -------------BRIGHTNESS--------##########
lbl_brightness = Label(RHS, text='Brightness:', font=(
    'arial', 12, 'bold'), bg="#f4f5f5")
...
def weather():
    ...
    textfield = tk.Entry(app1, justify='center', width=14, font=(
        'poppins', 23, 'bold'), bg='#404040', border=0, fg='white')
    textfield.place(x=69, y=31)
    textfield.focus()

    Search_icon = PhotoImage(file='Image/search_icon.png')
    myimage_icon = Button(app1, image=Search_icon,
                          borderwidth=0, cursor="hand2", bg='#404040', command=getWeather)
    myimage_icon.place(x=387, y=31)
...
def clock():
    ...
    lbl = Label(app2, font=('digital-7', 50, 'bold'),
...
def calendar():
    app3 = Toplevel()
    app3.geometry('500x500+510+210')
    app3.title("CALENDAR")
    app3.configure(width=400, height=400, bg='#292e2e')
    app3.resizable(False, False)

    image_icon = PhotoImage(file="Image/App3.png")
    app3.iconphoto(False, image_icon)

    mycal = Calendar(app3, setmode='day', date_pattern='d/m/yyyy')
    mycal.pack(fill='both', expand=True, padx=25, pady=25)
...
def game():
    ...
    def roll():
        dice = ['\u2680', '\u2681', '\u2682', '\u2683', '\u2684', '\u2685']
        label.configure(
            text=f'{random.choice(dice)}{random.choice (dice)}', fg="#29232e", font=("Arial", 130))
        label.pack()
...
def file():
    subprocess.Popen(r'explorer /select, "C:\Users\hp\Downloads\"')
...
###############################################################
RHB = Frame(Body, width=490, height=230, bg="#f4f5f5",
            highlightbackground="#adacb1", highlightthickness=1)
RHB.place(x=455, y=305)
